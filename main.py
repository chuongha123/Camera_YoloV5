...
output_frame = None
lock = threading.Lock()
frame_queue = queue.Queue(maxsize=2)  # Giới hạn queue để tránh lag
...
# Lưu trữ các box đã phát hiện để làm mượt
detected_objects = []
...
def detect_vehicles():
    global output_frame, detected_objects
    ...
    while True:
        ...
        if frame_count % DETECTION_INTERVAL == 0:
            ...
            update_detected_objects(new_detections)
...
def update_detected_objects(new_detections):
    global detected_objects
    
    # Xóa các đối tượng quá cũ (không còn xuất hiện trong 1 giây)
    current_time = time.time()
    detected_objects = [obj for obj in detected_objects if current_time - obj.last_seen < 1.0]
    ...
    for box, label, conf in new_detections:
        ...
        if not matched:
            detected_objects.append(DetectedObject(box, label, conf))
